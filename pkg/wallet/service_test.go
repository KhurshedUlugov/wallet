...
import (
        ...
        "github.com/KhurshedUlugov/wallet/pkg/types"
)
...
func TestSerivce_Repeat_notfound(t *testing.T) {
        svc := &Service{}
        acc, err := svc.RegisterAccount("+992000000001")
        fmt.Println(acc)
        fmt.Println(err)

        err = svc.Deposit(acc.ID, 100_000)
        fmt.Println(err)

        pmt, err := svc.Pay(acc.ID, 100, "cash")
        fmt.Println(pmt)
        fmt.Println(err)
        
        pmtn, err := svc.Repeat(pmt.ID)
        fmt.Println(pmtn)
        fmt.Println(err)
}
...
func TestService_FindFavoriteByID_FAIL(t *testing.T) {
        s := newTestService()
        _, err := s.FindFavoriteByID("fdffedfedf")
        if err == nil {
